...
public class Compiler {
  ...
  public Compiler(Protocol protocol) {
    this();
    // enqueue all types
   ...
   */
  public static void compileProtocol(File src, File dest,String template,String name) throws IOException {
    compileProtocol(new File[] { src }, dest,template,name);
  }
   ...
   * @param dest     the directory to place generated files in
   */
  public static void compileProtocol(File[] srcFiles, File dest,String template,String name) throws IOException {
    for (File src : srcFiles) {
      Protocol protocol = Protocol.parse(src);
      Compiler compiler = new Compiler(protocol,template,name);
      compiler.compileProtocolToDestination(src, dest);
    }
  }
  ...
  private void enqueue(Schema schema) {
    ...
    case BOOLEAN:
    case OBJECT:
      String realSchema = schema.getProp("realSchema");
  ...
  /** Generate output under dst, unless existing file is newer than src. */
  public void compileProtocolToDestination(File src, File dst) throws IOException {
    for (Schema schema : queue) {
      if(schema.getType() != (Schema.Type.EXTERNAL) && schema.getType() != (Schema.Type.GENERIC)) {
        OutputFile o = compile(schema);
        o.writeToDestination(src, dst);
  ...
  /** Generate output under dst, unless existing file is newer than src. */
  public void compileProtocolToDestination(File src, File dst,String template,String name) throws IOException {
    for (Schema schema : queue) {
      if(schema.getType() != (Schema.Type.EXTERNAL) && schema.getType() != (Schema.Type.GENERIC)) {
        OutputFile o = compile(schema);
        o.writeToDestination(src, dst);
  ...
  protected int calcAllArgConstructorParameterUnits(Schema record) {
    ...
    if (record.getType() != Schema.Type.RECORD && record.getType() != Schema.Type.EXTERNAL && record.getType() != Schema.Type.GENERIC)
  ...
  private Protocol addStringType(Protocol p) {
    ...
    // annotate messages
    Map<String, Protocol.Message> newM = newP.getMessages();
  ...
  private Schema addStringType(Schema s, Map<Schema, Schema> seen) {
    ...
    case RECORD:
      ...
      seen.put(s, result);
      List<Schema.Field> newFields = new ArrayList<>(s.getFields().size());
...
*/
  ...
  public Schema primitiveTypeMap(Schema t){
    switch(t.getName()){
      ...
      case "java.lang.Integer": return Schema.create(Schema.Type.INT);
      default: return t;
    }

  }

  public String javaUnboxShort(Schema schema, boolean firstLetterUpCase) {
    if(firstLetterUpCase){
      String str =  schema.getName().toLowerCase().substring(schema.getName().lastIndexOf(".")+1);
