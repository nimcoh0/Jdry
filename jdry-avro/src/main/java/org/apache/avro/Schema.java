 ...
 * adding support for OBJECT, EXTERNAL, INTEGER,GENERIC,VOID
 * EXTERNAL : external type . no record need to be created
 * GENERIC : support for java generic type
 * <p>
...
public abstract class Schema extends JsonProperties implements Serializable {
  ...
  public enum Type {
    RECORD, ENUM, ARRAY, MAP, UNION, FIXED, STRING, BYTES, INT, LONG, FLOAT, DOUBLE, BOOLEAN, NULL, OBJECT, EXTERNAL, INTEGER,GENERIC,VOID;
    private String name;
  ...
  static {
    ...
    ENUM_RESERVED.add("boot");
    ENUM_RESERVED.add("component");
    ENUM_RESERVED.add("strategy");
    ENUM_RESERVED.add("expression");
    ENUM_RESERVED.add("assertThat");
    ENUM_RESERVED.add("execType");
  }

  int hashCode = NO_HASHCODE;

  public boolean isGeneric(){
    if(this.getType().name.equals("generic")){
      return true;
    }
    return false;
  }
   ...
   *             by {@link Parser#parse(String)}. It will be removed in a
  ...
  public boolean isPrimitive(){
    if(PRIMITIVES.containsKey(this.getName().toLowerCase())){
      return true;
    }
    return false;
  }
  ...
  public static class Field extends JsonProperties {
    ...
    public void setDefaultValue(JsonNode str) {
  ...
  public static class ExternalSchema extends NamedSchema {
    ...
    public ExternalSchema(Name name, String doc, boolean isError) {
  ...
  private static class GenericSchema extends NamedSchema {
    private List<Field> fields;
    private Map<String, Field> fieldMap;
    private final boolean isError;

    public GenericSchema(Name name, String doc, boolean isError) {
      super(Type.GENERIC, name, doc);
      this.isError = isError;
    }


    public GenericSchema(Name name, String doc, boolean isError, List<Field> fields) {
      super(Type.GENERIC, name, doc);
      this.isError = isError;
      setFields(fields);
    }
    ...
    void toJson(Names names, JsonGenerator gen) throws IOException {
      ...
      String savedSpace = names.space; // save namespace
      gen.writeStartObject();
      gen.writeStringField("type", isError ? "error" : "generic");
      writeName(names, gen);
      names.space = name.space; // set default namespace
      if (getDoc() != null)
        gen.writeStringField("doc", getDoc());
  ...
  private static class VoidSchema extends Schema {
    public VoidSchema() {
      super(Type.VOID);
    }
  }
  ...
  static class Names extends LinkedHashMap<Name, Schema> {
    ...
    public Schema get1(String o) {
      Type element = PRIMITIVES.get(o);
      if (element != null) {
        return Schema.create(element);
      }
  ...
  static Schema parse(JsonNode schema, Names names) {
    if (schema == null) {
      ...
      Schema result = null;
      Name tmpName = new Name(schema.textValue(),"");
      if(names.isGeneric(tmpName)){
       // result = new GenericSchema(tmpName,null,false);
        result = names.get1(schema.textValue());
      }else {
        result = names.get(schema.textValue());
      }
      //Schema result = schema.textValue();
      if (result == null)
      ...
      String doc = null;
      if (type.equals("record") || type.equals("error") || type.equals("enum") || type.equals("fixed") || type.equals("object") || type.equals("external")  ) {
      ...
      if(type.equals("generic")){
        ...
        names.space(name.toString()); // set default namespace
      }
      if (PRIMITIVES.containsKey(type)) { // primitive
        ...
        result.setFields(fields);
      } else if (type.equals("external")) { // record
        //List<Field> fields = new ArrayList<>();
        List<Field> fields = new ArrayList<>();
        result = new ExternalSchema(name, doc, type.equals("error"));
        if (name != null)
          names.add(result);
        JsonNode fieldsNode = schema.get("fields");
        if (fieldsNode == null || !fieldsNode.isArray()){
          fieldsNode =(ObjectNode) MAPPER.createObjectNode();
          //fieldsNode = new JSONArray();
        }
          //throw new SchemaParseException("Record has no fields: " + schema);
          for (JsonNode field : fieldsNode) {
            String fieldName = getRequiredText(field, "name", "No field name");
        ...
        //List<Field> fields = new ArrayList<>();
        List<Field> fields = new ArrayList<>();
        result = new GenericSchema(name, doc, type.equals("error"));
        if (name != null)
          names.add(result);
        JsonNode fieldsNode = schema.get("fields");
        if (fieldsNode == null || !fieldsNode.isArray()){
